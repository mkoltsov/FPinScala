...
println(ls.foldLeft(stringMonoid.zero)(stringMonoid.op))
println(ls.foldRight(stringMonoid.zero)(stringMonoid.op))
...
// A function having the same argument and return type is sometimes called an endofunc- tion
...
// Sometimes there will be a homomorphism in both directions between two monoids. If they satisfy a monoid isomorphism (iso- meaning equal), we say that the two monoids are isomorphic

trait Foldable[F[_]] {
def foldRight[A,B](as: F[A])(z: B)(f: (A,B) => B): B def foldLeft[A,B](as: F[A])(z: B)(f: (B,A) => B): B
def foldMap[A,B](as: F[A])(f: A => B)(mb: Monoid[B]): B def concatenate[A](as: F[A])(m: Monoid[A]): A =
